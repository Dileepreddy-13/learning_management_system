...
const CourseDetails = () => {
  ...
  const { allCourses, calculateRating, calculateChapterTime, calculateCourseTime, calculateTotalLectures, currency, backendURL, userData, getToken } = useContext(AppContext)
  ...
  useEffect(() => {
    if (courseData && userData) {
      ...
      setIsEnrolled(enrolledCourseIds.includes(courseData._id.toString()))
    }
  }, [courseData, userData])
    ...
    <>
      <div className='flex md:flex-row flex-col-reverse gap-10 relative  items-start justify-between md:px-36 px-8 md:pt-32 pt-20 text-left'>
        ...
        <div className='max-w-xl z-10 text-gray-500'>
          <h1 className='md:text-4xl text-3xl font-semibold text-gray-800'>{courseData.courseTitle}</h1>
          <p dangerouslySetInnerHTML={{ __html: courseData.courseDescription }} className='mt-3 rich-text'></p>

          <div className='flex items-center space-x-2 mt-3 mb-1 text-sm'>
            ...
            <div className='mt-5'>
              {
                ...
                (
                  <div key={index} className='border border-gray-300 bg-white mb-2 rounded' onClick={() => toggleSection(index)}>
                    <div className='flex items-center justify-between px-4 py-3 cursor-pointer select-none'>
                      <div className='flex items-center gap-2'>
                        <img src={assets.down_arrow_icon} alt="arrow icon" className={`transform transition-transform duration-300 ${openSections[index] ? 'rotate-180' : ''}`} />
        ...
        <div className='max-w-[420px] z-10 shadow-[0px_4px_15px_2px_rgba(0,0,0,0.1)] rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px]   sm:min-w-[420px]'>
          ...
          <div className='p-5'>
            ...
            <div className='flex items-center text-sm md:text-default gap-4 mt-2 md:mt-4 text-gray-500'>
