...
const AddCourse = () => {
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-lg text-gray-500'>
        ...
        <div className='flex items-center justify-between flex-wrap'>
          ...
          <div className='flex md:flex-row flex-col items-center gap-3'>
            ...
            <label htmlFor="thumbnailImage" className='flex items-center gap-3'>
              <img src={assets.file_upload_icon} alt="thumbnailImage" className='p-3 bg-blue-500 rounded' />
        ...
        <div>
          {
            chapters.map((chapter, chapterIndex) => (
              <div key={chapterIndex} className='bg-white border rounded-md mb-3 shadow-sm'>
                <div className='flex justify-between items-center px-4 py-2 border-b'>
                  <div className='flex items-center'>
                    <img src={assets.dropdown_icon} alt="dropdownIcon" className={`w-4 h-4 mr-2 cursor-pointer transition-all ${chapter.collapsed && "-rotate-90"}`} onClick={() => handleChapter('toggle', chapter.chapterId)} />
                    <span className='font-semibold text-sm'>{chapterIndex + 1}. {chapter.chapterTitle}</span>
                  </div>
                  <span className='text-gray-400 text-sm'>{chapter.chapterContent.length} Lectures</span>
                  <img src={assets.cross_icon} alt="crossIcon" className='cursor-pointer' onClick={() => handleChapter('remove', chapter.chapterId)} />
                </div>
                {
                  !chapter.collapsed && (
                    <div className='p-4'>
                      {chapter.chapterContent.map((lecture, lectureIndex) => (
                        <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                          ...
                          <img src={assets.cross_icon} alt="crossIcon" className='cursor-pointer' onClick={() => handleLecture('remove', chapter.chapterId, lectureIndex)} />
                        </div>
                      ))}
            ...
            showPopup && (
              <div className='fixed inset-0 flex items-center justify-center bg-black/30 backdrop-blur-sm'>
                <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
                  ...
                  <img onClick={e => setShowPopup(false)} src={assets.cross_icon} alt="crossIcon" className='absolute top-4 right-4 w-4 cursor-pointer' />
                </div>
              </div>
            )
          }
        </div>
        <button type='submit' className='bg-black text-white w-max py-2.25 px-8 rounded my-4 cursor-pointer'>Add</button>
      </form>
    </div>
  )
}
